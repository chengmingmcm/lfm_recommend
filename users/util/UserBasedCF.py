...
class UserBasedCF(object):
    ...
    def predict(self, user, K=20, N=None):
        """
        对输入的一个user进行推荐
        :param K:
        :param N:
    ...
    def validate(self, K=20):
        ...
        for user in self.test:
            ...
            recommendation = self.predict(1003)  # ->list
...
if __name__ == '__main__':
    ...
    print("准确率:", pre, "召回率", rec)
    pre_rec.append((pre, rec))

    result_list = usercf.predict(1003)
    for i in result_list[:recom_num]:
        matrix.append(i[0])
    print(matrix)